...
$sql = "SELECT * FROM proizvodi WHERE kategorija_id = 1 AND aktivan = 1";
...
<!-- da proverimo da li je admin, korisnik ili gost -->
...
<!--AJAXX !!!!!!!!!!!!!!!-->
<button class="btn btn-primary dodaj-korpu" data-id="<?php echo $row['id']; ?>">Kupi</button>                              <?php else: ?>
...
<script>
document.querySelectorAll('.dodaj-korpu').forEach(btn => {
    btn.addEventListener('click', function(e){
        e.preventDefault();
        let id = this.dataset.id;

        fetch('ajax_dodaj_korpu.php?id=' + id)
        .then(response => response.json())
        .then(data => {
            // Ažuriraj broj proizvoda u korpi
            let spanKorpa = document.getElementById('broj-korpe');
            if(spanKorpa){
                spanKorpa.innerText = data.ukupno;
            }

            // Prikazi kratku poruku  - prikazuje iznad nav bara
            let poruka = document.createElement('div');
            poruka.className = 'alert alert-success mt-2';
            poruka.innerText = "Proizvod je dodat u korpu!";
            document.body.prepend(poruka);

            setTimeout(() => {
                poruka.remove();
            }, 2000);

            // Opcionalno: promeni tekst dugmeta na "Dodato!" na par sekundi
            let original = this.innerText;
            this.innerText = "Dodato!";
